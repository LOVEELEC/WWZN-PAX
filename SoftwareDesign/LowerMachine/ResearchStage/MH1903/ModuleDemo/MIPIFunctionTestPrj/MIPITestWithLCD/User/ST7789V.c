...
{
    /************* display Setting *************/
    LCD_WriteEx(LCDI_CMD, LCD_CMD_MADCTL);      // 36H Memory Data Access Control
    LCD_WriteEx(LCDI_DAT, 0x00);                
	
    LCD_WriteEx(LCDI_CMD, LCD_CMD_COLMOD);      // 3AH Interface Pixel Format
//    LCD_WriteEx(LCDI_DAT, 0x05);                // < 16bit/pixel
	LCD_WriteEx(LCDI_DAT, 0x60); 
	/*********** Frame Rate Setting ***********/
    LCD_WriteEx(LCDI_CMD, LCD_CMD_PORCTRL);     // B2H Porch Setting
    ...
    LCD_WriteEx(LCDI_CMD, LCD_CMD_GCTRL);       // B7H Gate Control
    LCD_WriteEx(LCDI_DAT, 0x35);                // < VGH:13.26, VGL:-10.43
	
    LCD_WriteEx(LCDI_CMD, LCD_CMD_VCOMS);       // BBH VCOM Setting 
    LCD_WriteEx(LCDI_DAT, 0x35);                // < VCOM: 1.425
	
    LCD_WriteEx(LCDI_CMD, LCD_CMD_LCMCTRL);     // C0H LCM Control
    LCD_WriteEx(LCDI_DAT, 0x2c);                
	
    LCD_WriteEx(LCDI_CMD, LCD_CMD_VDVVRHEN);    // C2H VDV and VRH Command Enable
    LCD_WriteEx(LCDI_DAT, 0x01);
	LCD_WriteEx(LCDI_DAT, 0xFF);
	
    LCD_WriteEx(LCDI_CMD, LCD_CMD_VRHS);        // C3H VRH Set <调整电压参数>
    LCD_WriteEx(LCDI_DAT, 0x05);                // < 3.8+
	
    LCD_WriteEx(LCDI_CMD, LCD_CMD_VDVS);        // C4H VDV Set
    LCD_WriteEx(LCDI_DAT, 0x20);                // < VDV: 0V
	
    LCD_WriteEx(LCDI_CMD, LCD_CMD_FRCTRL2);     // C6H Frame Rate Control in Normal Mode 
    LCD_WriteEx(LCDI_DAT, 0x0f);                // < 60Hz
	
    LCD_WriteEx(LCDI_CMD, LCD_CMD_PWCTRL1);     // D0H Power Contril 1
    LCD_WriteEx(LCDI_DAT, 0xa4);                // 
    LCD_WriteEx(LCDI_DAT, 0xa1);                // < AVDD:6.8V, AVCL:-4.8V, VDDS:2.3V
	
	/************* Gamma Setting **************/
    LCD_WriteEx(LCDI_CMD, LCD_CMD_PVGAMCTRL);   // E0H Positive Voltage Gamma Control
    ...
    LCD_WriteEx(LCDI_CMD, LCD_CMD_NVGAMCTRL);   // E1H Negative Voltage Gamma Control
    LCD_WriteEx(LCDI_DAT, 0xd0);
    LCD_WriteEx(LCDI_DAT, 0x00);
    LCD_WriteEx(LCDI_DAT, 0x05);
    LCD_WriteEx(LCDI_DAT, 0x0d);
...
void LCD_DisplayImageDirect(const uint8_t *picture, uint32_t size)
{
    uint32_t i;
	uint32_t col = 192;
    uint32_t row = size / col;
    LCD_BlockWritePrep(0, col - 1, 0, row - 1);
...
void LCD_DisplaySetSizeImageDirect(const uint8_t *picture, uint32_t col, uint32_t row)
{
    uint32_t i;
//	uint32_t col = 192;
//    uint32_t row = size / col;
...
void LCD_DisplayImageDMA(uint8_t *picture, uint32_t size)
{
    uint32_t i;
	uint32_t col = 192;
    uint32_t row = size / col;
    ...
    LCD_DMAInit(picture, (uint8_t)(col)); 
    ...
    for (i = 0; i < row; i++)
    {
        ...
        LCD_DMAWriteEnable(picture + i*col);
    }
  	LCD_CS_HIGH();  
}
