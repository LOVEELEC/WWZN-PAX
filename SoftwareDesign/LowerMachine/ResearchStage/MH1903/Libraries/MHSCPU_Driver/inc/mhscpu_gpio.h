...
#define GPIO_PortSourceGPIOG       ((uint32_t)0x06)
#define GPIO_PortSourceGPIOH       ((uint32_t)0x07)
...
typedef enum
{
	GPIO_WakeMode_Off				= (uint32_t)0x00,
	GPIO_WakeMode_Now				= (uint32_t)0x01,
	GPIO_WakeMode_AfterGlitch		= (uint32_t)0x02
}GPIO_WakeModeTypeDef;

#define IS_GPIO_WAKE_MODE(MODE)		(((MODE) == GPIO_WakeMode_Off) || \
									((MODE) == GPIO_WakeMode_Now) || \
